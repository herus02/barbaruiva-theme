<?php
/**
 * Page titles
 */
function roots_title() {
  if (is_home()) {
      return get_the_title(get_option('page_for_posts', true));

    if (get_option('page_for_posts', true)) {
      return get_the_title(get_option('page_for_posts', true));
    } else {
      return __('Latest Posts', 'roots');
    }
  } elseif (is_archive()) {
    $term = get_term_by('slug', get_query_var('term'), get_query_var('taxonomy'));
    if ($term) {
      $obj = get_post_type_object(get_post_type());
      $title = apply_filters('the_title', $obj->labels->name);
      $title .= " / ";
      $title .= apply_filters('single_term_title', $term->name);
      return $title;
    } elseif (get_post_type() != 'post') {
      $obj = get_post_type_object(get_post_type());
      return apply_filters('the_title', ucfirst($obj->taxonomies[0]));
    } elseif (is_post_type_archive()) {
      return apply_filters('the_title', 'Blog');
    } elseif (is_day()) {
      return sprintf(__('Daily Archives: %s', 'roots'), get_the_date());
    } elseif (is_month()) {
      return sprintf(__('Monthly Archives: %s', 'roots'), get_the_date('F Y'));
    } elseif (is_year()) {
      return sprintf(__('Yearly Archives: %s', 'roots'), get_the_date('Y'));
    } elseif (is_author()) {
      $author = get_queried_object();
      return sprintf(__('Author Archives: %s', 'roots'), apply_filters('the_author', is_object($author) ? $author->display_name : null));
    } else {
      $obj = get_post_type_object(get_post_type());
      $title = apply_filters('the_title', $obj->labels->name);
      $title .= " / ";
      $title .= single_cat_title('', false);
      return $title;
    }
  } elseif (is_single()) {
      $obj = get_post();
      $tax = get_object_taxonomies(get_post_type());
      $term = get_the_term_list(get_the_id(),$tax);
      $p_type = get_post_type_object($obj->post_type);

      $title .= apply_filters('single_term_title', $term);
      $title .= " / ";
      $title .= apply_filters('the_title', get_the_title());
      return $title;
    
  } elseif (is_search()) {
    return sprintf(__('Search Results for %s', 'roots'), get_search_query());
  } elseif (is_404()) {
    return __('Not Found', 'roots');
  } else {
    return get_the_title();
  }
}
